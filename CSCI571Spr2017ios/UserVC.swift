...
class UserVC: UIViewController, UITableViewDataSource{
    ...
    //TODO test that pagenation work by looking at entire output
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request("http://csci571spr2017ypios-env.us-west-2.elasticbeanstalk.com/wHW92ag5.php?q="+queryStr+"&type=user").responseJSON{ response in
            self.fetched = true
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(numRow > 0 && searchData.count>0)
        {
            ...
            let favList = UserDefaults.standard.dictionary(forKey: "user")
            if(favList == nil)
            {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        albumView.detailName = "user"
        let postView = newView.viewControllers![1] as! PostVC
        postView.detailID = thisSender.profileID
        postView.detailName = "user"
        postView.profileURL = thisSender.profileURL
 
        /*
        let NalbumView = newView.viewControllers![0] as! UINavigationController
        let albumView = NalbumView.topViewController as! AlbumVC
        albumView.detailID = thisSender.profileID
        let NpostView = newView.viewControllers![1] as! UINavigationController
        let postView = NpostView.topViewController as! PostVC
        postView.detailID = thisSender.profileID
 */
    }
